...
SnapOrder by Lineweb gives restaurants a self-hosted food menu and ordering workflow inside WordPress. There are no commissions, paid feature gates, trials, or Pro upsells.
...
SnapOrder works without an external service when cash payment is used. The following optional integrations are contacted only after a site administrator enables and configures them.

= Stripe =

Stripe is used to process card payments. The browser loads Stripe.js from `https://js.stripe.com/` and sends card details directly to Stripe. SnapOrder's server sends the authoritative order amount, currency, SnapOrder order ID, and an idempotency key to `https://api.stripe.com/`. Stripe returns payment identifiers and statuses; SnapOrder does not store full card details.
...
Twilio is used only when WhatsApp notifications are enabled. SnapOrder sends the configured sender number, customer phone number, and the relevant order/status message to `https://api.twilio.com/`.
...
SnapOrder does not send telemetry, usage analytics, or marketing data to Lineweb or the plugin author.
...
SnapOrder is created and maintained by Andrew Matia / Lineweb.

== Installation ==

1. Upload the `snaporder` folder to `/wp-content/plugins/`, or install the ZIP from Plugins > Add New > Upload Plugin.
2. Activate SnapOrder.
3. Add categories and food items from the WordPress admin menu.
4. Open SnapOrder settings and configure branding, opening hours, order types, and at least one payment method.
...
For Stripe, add the webhook URL displayed in SnapOrder settings to the Stripe dashboard and subscribe it to `payment_intent.succeeded`, `payment_intent.payment_failed`, and `payment_intent.canceled`.

== Development and source ==

SnapOrder ships its runtime assets locally. To reproduce them, install Node.js 20 or newer, run `npm ci`, and then run `npm run build` from the plugin directory.
...
No. SnapOrder is a complete free plugin. It contains no Pro upgrade screen, trial, remote feature lock, or upsell notice.

= Does SnapOrder trust prices submitted by the browser? =
...
= Does SnapOrder delete data when it is removed? =
